...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            image = self.augmentations(image) if random.random() > 0.08 else image
            ...
            for annotation in annotations[:self.length]:
                ...
                coord_points = annotation['coords']
                num_coords_points = len(coord_points)
                points.append(coord_points)
                coord_point_labels.append([count_label] * num_coords_points)
            ...
            coord_points = self.transform.apply_coords_torch(points, original_input_size)
            bboxes = self.transform.apply_boxes_torch(bboxes, original_input_size)
            coord_points = np.stack(coord_points, axis=0)
            category_ids = np.stack(category_ids, axis=0)
            bboxes = np.stack(bboxes, axis=0)
            masks = np.stack(masks, axis=0) 
            point_labels = np.stack(coord_point_labels, axis=0)
    ...
    def collate_fn(cls, batch):
        images, bboxes, masks, center_points, point_labels ,original_input_size, resized_input_size, coco_image_names = zip(*batch)
        images = torch.stack(images, dim=0)
        return images, bboxes, masks, center_points, point_labels, original_input_size, resized_input_size, coco_image_names
