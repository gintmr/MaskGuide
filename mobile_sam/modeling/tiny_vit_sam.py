...
class PatchEmbed(nn.Module):
    def __init__(self, in_chans, embed_dim, resolution, activation):
        ...
        self.seq = nn.Sequential(
            Conv2d_BN(in_chans, n // 2, 3, 2, 1), # 3, 32, 3, 2, 1
            activation(),
            Conv2d_BN(n // 2, n, 3, 2, 1), # 32, 64, 3, 2, 1
        )
...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        patches_resolution = self.patch_embed.patches_resolution # 1024/4
        ...
        for i_layer in range(self.num_layers):
            kwargs = dict(dim=embed_dims[i_layer], # 64,128,160,320
                        input_resolution=(patches_resolution[0] // (2 ** (i_layer-1 if i_layer == 3 else i_layer)), # 256/1, 256/2, 256/4, 256/4
                        ...
                        #                     patches_resolution[1] // (2 ** i_layer)),
                          ...
                          out_dim=embed_dims[min(
                              i_layer + 1, len(embed_dims) - 1)], # 128, 160, 320, 320
                          activation=activation,
                          )
            if i_layer == 0:
                layer = ConvLayer(
    ...
    def forward_features(self, x):
        ...
        x = self.layers[0](x) # ([1, 16384, 128])
        start_i = 1

        features = {}
        for i in range(start_i, len(self.layers)):
            # print(f"the {i}th layer!!!!!!!!!!!!")
            layer = self.layers[i]
            x = layer(x)
...
class tiny_TinyViT(TinyViT):
    ...
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        super().__init__(img_size, in_chans, num_classes, embed_dims, depths, num_heads, window_sizes, mlp_ratio, drop_rate, drop_path_rate, use_checkpoint, mbconv_expand_ratio, local_conv_size, layer_lr_decay)
        self.neck = nn.Sequential(
            nn.Conv2d(
                embed_dims[-1],
                256,
