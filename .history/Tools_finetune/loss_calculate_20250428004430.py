...
class OceanSegmentationLoss:
    ...
    def __call__(self, pred_masks, ori_masks):
        ...
        for i in range(pred_probs.size(0)):  # 按样本分块，chunksize为1
            ...
            # 损失计算（明确输入类型）
            iou_loss_sum += self._calculate_iou_loss(pred_prob, ori_mask)         # 需归一化概率
            dice_loss_sum += self._calculate_dice_loss(pred_prob, ori_mask)       # 需归一化概率
            tversky_loss_sum += self._calculate_tversky_loss(pred_prob, ori_mask) # 需归一化概率
        ...
        results.update({'iou_loss': iou_loss_sum / pred_probs.size(0),
