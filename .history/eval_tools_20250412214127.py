...
def calculate_segmentation_losses(pred_masks, ori_masks, include_dice_iou=True):
    ...
    for i in range(0, len(pred_masks), chunk_size):
        ...
        chunk_pred_masks = F.interpolate(chunk_pred_masks, scale_factor=0.5, mode='bilinear', align_corners=False)
        chunk_ori_masks = F.interpolate(chunk_ori_masks, scale_factor=0.5, mode='bilinear')
