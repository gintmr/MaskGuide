...
from .modeling import ImageEncoderViT, MaskDecoder, PromptEncoder, Sam, TwoWayTransformer, TinyViT, tiny_TinyViT
...
def init_weights_by_mobilesam(model):
    ...
    # 加载 prompt_encoder 权重
    model.prompt_encoder.load_state_dict(mobilesam_weight['prompt_encoder'])
    # 加载 mask_decoder 权重
    model.mask_decoder.load_state_dict(mobilesam_weight['mask_decoder'])
...
def build_tiny_msam(checkpoint=None):
    ...
    if os.environ['MODEL_MODE'] == "test":
        mobile_sam = Sam(
                image_encoder=tiny_TinyViT(img_size=1024, in_chans=3, num_classes=1000,
        ...
        if checkpoint is not None:
            ...
            mobile_sam.apply(init_weights_by_mobilesam)
...
def check_missing_layers(model, state_dict):
    ...
    # 找出权重文件中模型结构中不存在的层
    missing_layers = state_dict_keys - model_keys

    if missing_layers:
        print("权重文件中以下层在模型结构中不存在：")
        for layer in missing_layers:
            print(f"  {layer}")
    else:
        print("权重文件中的所有层都存在于模型结构中")
