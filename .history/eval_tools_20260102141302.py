...
def calculate_segmentation_losses(pred_masks, ori_masks, include_dice_iou=True):
    ...
    for i in range(0, len(pred_masks), chunk_size):
        ...
        chunk_pred_masks = F.interpolate(chunk_pred_masks.unsqueeze(0), scale_factor=0.5, mode='bilinear').squeeze(0)
        chunk_ori_masks = F.interpolate(chunk_ori_masks.unsqueeze(0), scale_factor=0.5, mode='bilinear').squeeze(0)
...
def clean_checkpoint_path(check_point_path, train=False):
    if train:
        temp_path = '/data2/wuxinrui/Distill-SAM/weights/temp_weights/train.pth'
    else:
        temp_path = '/data2/wuxinrui/Distill-SAM/weights/temp_weights/test.pth'
