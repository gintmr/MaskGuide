...
def calculate_segmentation_losses(pred_masks, ori_masks, include_dice_iou=True):
    ...
    for i in range(0, len(pred_masks), chunk_size):
        ...
        print(f"(chunk_pred_masks).shape = {(chunk_pred_masks.shape)}")
        print(f"(chunk_ori_masks).shape = {(chunk_ori_masks.shape)}")
        ...
        ce_loss += F.binary_cross_entropy_with_logits(
            chunk_pred_masks, chunk_ori_masks, reduction='meam')
