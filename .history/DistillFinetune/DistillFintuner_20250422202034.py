...
class AbstractDistillFinetuner(pl.LightningModule, ABC):
    ...
    def feature_distillation_loss(self, T_features, S_features, T_layers_features, S_layers_features, RATE, reduction='mean', alpha=0.8, beta=0.2):
        ...
        返回:
            torch.Tensor: 蒸馏损失。
        """
        assert not torch.isnan(T_features).any(), "T_features contains NaN"
        assert not torch.isinf(T_features).any(), "T_features contains Inf"
        assert not torch.isnan(S_features).any(), "S_features contains NaN"
        assert not torch.isinf(S_features).any(), "S_features contains Inf"
        ...
        for T_layers_feature, S_layers_feature in zip(T_layers_features.values(), S_layers_features.values()):
            ...
            layers_kl_div = torch.sum(layers_kl_div)
