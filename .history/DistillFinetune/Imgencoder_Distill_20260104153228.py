...
class Imgencoder_Distill(AbstractDistillFinetuner):
    ...
    def __init__(
        ...
        # 注意：'reduce-overhead' 模式使用 CUDA graphs，在循环中可能出现兼容性问题
        # 如果遇到 CUDA graphs 错误，设置环境变量 USE_TORCH_COMPILE=0 禁用编译
        self._use_compile = hasattr(torch, 'compile') and os.getenv('USE_TORCH_COMPILE', '0') == '1'
        if self._use_compile:
            try:
                # 使用 'default' 模式避免 CUDA graphs 问题
                # 可选模式: 'default', 'reduce-overhead', 'max-autotune'
                ...
                if hasattr(self.S_model, 'mask_decoder'):
                    # 使用 'default' 模式，避免 'reduce-overhead' 模式中的 CUDA graphs 问题
                    self.S_model.mask_decoder = torch.compile(
                        self.S_model.mask_decoder, 
                        mode=compile_mode,
                        fullgraph=False  # 允许部分图编译，更灵活
                    )
                if hasattr(self.S_model, 'prompt_encoder'):
                    self.S_model.prompt_encoder = torch.compile(
                        self.S_model.prompt_encoder, 
                        mode=compile_mode,
                        fullgraph=False
                    )
                print(f"Successfully compiled model with mode={compile_mode}")
            except Exception as e:
                print(f"Warning: torch.compile failed, falling back to normal mode: {e}")
                print(f"To disable compilation, set USE_TORCH_COMPILE=0")
    ...
    def forward(self, imgs, bboxes, labels, center_points, point_labels, original_input_size, resized_input_size, coco_image_names, validate=False):
        ...
        try:
            ...
            if self.only_distill:
                random_pobs = random.random()
                if random_pobs < 0.05:
                    self.only_distill = False
