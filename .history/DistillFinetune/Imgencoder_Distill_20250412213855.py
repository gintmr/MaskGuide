        ...
        try:
            with autocast():
                ...
                for T_feature, S_feature, bbox, label, center_point, point_label, target_label, original_input_size_item, resized_input_size_item, coco_image_name, img in \
                    ...
                    penalty_coefficient = 1.2 + np.log2(single_img_iou)
                    single_LOSS = (single_img_loss_focal + 1.5 * single_img_loss_dice + 2.5 * single_img_loss_IoU) / penalty_coefficient if single_img_iou >= 0.5 else 0
                    #g 排除低质样本的副作用
                    BS_LOSS += single_LOSS
            ...
            return {
                ...
                'loss': (BS_LOSS + distill_loss * self.distill_weight)  / penalty_coefficient,
