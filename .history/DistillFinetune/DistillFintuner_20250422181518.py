...
class AbstractDistillFinetuner(pl.LightningModule, ABC):
    ...
    def feature_distillation_loss(self, T_features, S_features, T_layers_features, S_layers_features, RATE, reduction='mean', alpha=0.8, beta=0.2):
        """
        计算教师模型和学生模型特征的蒸馏损失。
        ...
        epsilon = 1e-9
        student_softmax = F.softmax(S_features / temperature, dim=1).clamp(min=epsilon)
        teacher_softmax = F.softmax(T_features / temperature, dim=1).clamp(min=epsilon)
        ...
        for T_layers_feature, S_layers_feature in zip(T_layers_features.values(), S_layers_features.values()):
            ...
            s_layer_softmax = F.softmax(S_layers_feature / temperature, dim=1).clamp(min=epsilon)
            t_layer_softmax = F.softmax(T_layers_feature / temperature, dim=1).clamp(min=epsilon)
