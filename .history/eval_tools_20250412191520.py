...
def calculate_segmentation_losses(pred_masks, ori_masks, include_dice_iou=True):
    ...
    chunk_size = 5
    # 在计算损失之前同步数据
    pred_masks = sync_tensor(pred_masks)
    ori_masks = sync_tensor(ori_masks)
    ...
    pred_masks_by_chunk = [pred_masks[i:i+chunk_size] for i in range(0, len(pred_masks), chunk_size)]
    ori_masks_by_chunk = [ori_masks[i:i+chunk_size] for i in range(0, len(ori_masks), chunk_size)]
    for chunk_pred_masks, chunk_ori_masks in zip(pred_masks_by_chunk, ori_masks_by_chunk):
